    ...
    public ResponseEntity<RouteDto> updateRoute(RouteDto routeDto, Long id) {
        if (routeRepository.existsById(id)) {
            return routeRepository.findById(id).map(existingRoute -> {
                Optional.ofNullable(routeDto.getStartLocation()).ifPresent(existingRoute::setStartLocation);
                Optional.ofNullable(routeDto.getEndLocation()).ifPresent(existingRoute::setEndLocation);
...
//    }
    ...
    public ResponseEntity<List<RouteDto>> findRoutes(String startLocation, String endLocation) {
        List<RouteDto> routes = routeRepository.findByStartLocationAndEndLocation(startLocation, endLocation)
                .stream()
                .map(routeMapper::mapTo)
                .collect(Collectors.toList());
        return new ResponseEntity<>(routes, HttpStatus.OK);
